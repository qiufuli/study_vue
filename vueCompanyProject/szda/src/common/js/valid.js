...
const rules = {
    zzjgdm: /^[a-zA-Z0-9]{8}-[a-zA-Z0-9]|[a-zA-Z0-9]{18}$/, //组织机构代码
    full: /^[^\[\]:?\"{}`=^&!*|;$%@'<>()+\r\n,\\../ ]*$/,
    other: /^[^\[\]:?\"{}`=^&!*|;$%@'<>()+,\\../]*$/,
    normal: /^[^|;$%'<>`()+\r\n,\\../ *]*$/,
    titleLess: /^[^|$'<>`\"\\/*\\s*]*$/,
    ...
    lxfs: /^([0-9\(\)-])+$/, // 联系方式（包含手机号和固话-可()形式输入分机号）
    ...
    zkzhgz: /^[1-9][0-9]{3,5}$/,
    phone: /^((1\d{10})|((\d{3,4}-)?\d{6,8}(-\d{1,4})?))(、((1\d{10})|((\d{3,4}-)?\d{6,8}(-\d{1,4})?)))*$/,
    ajh: /^\d{4}-\d{4}$/,
    lah: /^[A-Z]\d{8}$/,
    jgz: /^[a-zA-Z0-9]{7,21}$/, //军官证
    hz: /^[a-zA-Z0-9]{3,21}$/ //护照
    // sfzhm:/^[1-9]\d{7}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}$|^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}([0-9]|X)$/
};
// 内置提示信息
const messages = {
    full: "含有非法字符",
...
function validFn(option, sync) {
    ...
    return (rule, value, callback) => {
        ...
        let length = 0;
        if (mat) {
            length = mat.length * 2 + (value.length - mat.length);
        } else {
            length = value.length;
        }
        if (length > optArr[0]) {
        ...
        if (!rules[check].test(value)) {
            ...
            if (typeof sync === "function") {
                return sync.call(null, rule, value, callback);
            }
            callback();
        }
    };
