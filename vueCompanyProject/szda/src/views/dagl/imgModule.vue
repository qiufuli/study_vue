<template>
  <div class="yxxx">
    <el-breadcrumb separator-class="el-icon-arrow-right" style="line-height:48px" class="noPrint">
      <i class="iconfont" style="float:left;margin-right:15px;">&#xe60c;</i>
      <el-breadcrumb-item >档案管理</el-breadcrumb-item>
      <el-breadcrumb-item >{{xm}}</el-breadcrumb-item>
    </el-breadcrumb>
    ...
    <el-dialog  :visible.sync="print_flag" class="print_wrap noPrint " :before-close="printClose">
      <el-form :model="print_data" :rules="rules" ref="print_form">
        ...
        <el-row>
          <el-col :span="12">
             <el-form-item label="打印用途：" prop="dyyt">
                <el-select v-model="print_data.dyyt" value-key="value" placeholder="请选择" class="wd" clearable>
                      <el-option
                      :label="item.dmmc"
                      :value="item.dmid"
         ...
         <el-row>
          <el-col :span="18">
             <el-form-item label="打印备注：" prop="bz" label-width="23%">
                <el-input v-model="print_data.bz" type="textarea" style="width:70%;resize:none;" :autosize="{ minRows: 2, maxRows: 3}"></el-input>
              </el-form-item>
          </el-col>
        </el-row>
        <el-row>
        ...
        <el-row class="print_select">
          <el-col >
             <el-radio v-model="getPage" label="1">打印当前页
                <el-select
                    v-model="getPageNum"
                    value-key="value"
                    placeholder="请选择"
        ...
        <el-row>
          <el-col>
              <el-radio v-model="getPage" label="2" prop="tpqsym">
                
                 <!-- 打印第<el-input :disabled="getPage==1"  class="page_input" size="mini" v-model="startNum"></el-input>页 至<el-input :disabled="getPage==1" class="page_input" v-model="endNum"></el-input>页 -->
                 打印第
                <el-autocomplete
                class="page_input"
        ...
        <el-row>
            <div class="btn_parent">
              <el-button type="primary" class="base_btn" style="line-height: 6px;" @click="print_click()" :disabled="print_disabled">打印</el-button>
            </div>
        </el-row>
      </el-form>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    querySearch(queryString, cb){
      ...
      var arr = new Array(_length).join(',').split(',').map((item,index)=>{ return {"value":(index+1).toString()}})
      cb(arr) 
    },
    // 关闭打印弹窗事件
    printClose(done){
    ...
    print_fn(){
      ...
      if(this.click_flag == 0){
        ...
        this.getPageNum = _data
        this.getPageNum_arr.push(_data);
      }else if(this.click_flag == 1){
        var _data1 = $('.J_left_img').attr('clsyxh');
        var _data2 = $('.J_right_img').attr('clsyxh')?$('.J_right_img').attr('clsyxh') : '';
        this.getPageNum = _data1
    ...
    chakan_menu_print(){
      this.menu_print_arr = this.zmlxx;
        this.$nextTick(()=>{
          $('.menu_print_class').removeClass('noPrint').addClass('showPrint');
          $('.print_div').removeClass('showPrint').addClass('noPrint');
          window.print();
        });
    },
    print_click(){
       this.$refs['print_form'].validate(valid =>{
        if(valid){
          if(this.getPage == 1){
            ...
            }else if(parseInt( this.startNum) > this.zys ||  parseInt(this.endNum) > this.zys){
          ...
          axios({
            ...
            if(_data.executeResult == 1){
               ...
               $('.print_div').removeClass('noPrint').addClass('showPrint');
               $('.menu_print_class').removeClass('showPrint').addClass('noPrint');
               this.$nextTick(()=>{
    ...
    create_print(print_tpxhs){
      ...
      for(var i=0;i<print_tpxhs.length;i++){
        var route_one = conf.urlPrefix + '/yxxx/tpxxcx?dayxid=' + this.dayxid + '&tpxh=' + print_tpxhs[i]+'&time='+Math.random();
        _arr.push(route_one);
      }
      return _arr;
    },
    goOther(){
      var _this = this;
        this.$nextTick(() => {
          _this.powerCheck("/dagl/sjxx", function() {
    ...
    j_menu() {
      if(this.dayxid){
        ...
        this.$message.warning("该人员暂无影像信息！");
      }
    },
    // 获取总目录
    all_menu(){
