          ...
          >
            ...
            >
              <template slot-scope="scope">
                <div class="ajh_class" @click="see(scope.row)">{{scope.row.lah}}</div>
              </template>
            </el-table-column>
            <el-table-column
              label="当事人"
            ...
            >
              <template slot-scope="scope" align="center">
                <el-button
                  ...
                  v-if="scope.row.ajztid=='61' || scope.row.ajztid=='69'"
                >审核</el-button>
              </template>
            </el-table-column>
          </el-table>
...
export default {
  ...
  methods: {
      ...
      this.$refs["xcdjForm"].validate(valid => {
        if (valid) {
          ...
          $({
            ...
            const _res = res.returnData;
            if (+_res.executeResult === 1) {
              this.tsjbsx_tableData = res.returnData.jaxashlb;
    ...
    see(item) {
      this.$router.push({
        name: "Jaspladj",
        query: { lasqbid: item.lasqbid, disable: true, lasqsp: true }
      });
    },
    examine(item) {
      // 通过wsid判断跳转结案还是销案
      if (item.wsid == '00000000000010') {
        this.$router.push({
          name: "Jasp",
          query: {
            id: item.id,
            laspbid: item.lasqbid,
            wsid: item.wsid,
            wslx: 2
          }
        });
