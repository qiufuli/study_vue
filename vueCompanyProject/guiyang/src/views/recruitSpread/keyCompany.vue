				...
				<el-row>
					...
					<el-col :span="6">
						<el-form-item label="单位规模" prop="dwgm">
							<select-code v-model="form.dwgm" code="gyrlzyw_qzzp_d_dwgm"></select-code>
						</el-form-item>
					</el-col>
					<el-col :span="6">
...
export default {
	...
	methods: {
		...
		getData() {
			this.loading = true;
			$.get('/gyrcht/zdqy/zdqy/zdqyCx', {
				params: this.formRight
			}).then((res) => {
				this.results = res.returnData.zdqylb;
		...
		postData() {
			// post 接口
			$.post('/gyrcht/zdqy/zdqy/zdqyBc', {
				dwxx_id: this.formDialog.dwxx_id
			}).then((res) => {
				if (res.returnData.executeResult === '1') {
					this.$message.success('重点企业添加成功！');
					this.getData();
				} else {
	...
	components: { selectCode, tradeSelect }
};
</script>
