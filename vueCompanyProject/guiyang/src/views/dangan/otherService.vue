		...
		<div class="loading" v-loading="loading">
			<div class="result_wrap">
				<el-table :data="results" stripe class="table_wrap width_100">
					...
					<el-table-column label="证明类型" prop="kjzmlx"></el-table-column>
					<el-table-column label="操作" width="210">
						<template scope="operate">
							<el-button title="审核" :type="operate.row.lczt == 10 ? 'primary' : ''" size="mini" icon="btn-approval"
								@click="approval(operate.row)"></el-button>
							<el-button :disabled="false"
								title="开证明" size="mini" :type="operate.row.lczt == 40 ? '' : 'primary'" class="icon-intro iconfont"
...
export default {
	name: 'otherService',
	components: { dateAss, selectCode, serviceCert },
	data() {
		return {
			labelWidth,
			color: {
	...
	methods: {
		...
		getData() {
			this.loading = true;
			$.get('/gyrcht/kjzm/kjzm/qtqkzmsqshCx', {
				params: this.form
			}).then((res) => {
				this.loading = false;
				this.results = res.returnData.sqshs;
