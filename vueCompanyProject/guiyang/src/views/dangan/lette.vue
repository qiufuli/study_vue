        ...
        <el-row>
          ...
          <el-col :span="6">
              ...
              <el-select v-model="form.lczt" class="width_100">
								...
								<el-option value="30" label="待开证明"></el-option>
								<el-option value="40" label="已开证明"></el-option>
								<el-option value="60" label="邮件发送失败"></el-option>
							</el-select>
						</el-form-item>
					</el-col>
        </el-row>
		...
		<div class="loading" v-loading="loading">
			<div class="result_wrap">
				<el-table :data="results" stripe class="table_wrap width_100">
					...
					<el-table-column label="申请日期" prop="sqsj"></el-table-column>
					<el-table-column label="流程状态" prop="ddhlcztmc"></el-table-column>
					<el-table-column label="操作" width="230">
						<template scope="operate">
							<el-button title="审核" :type="operate.row.ddhlczt == 10 ? 'primary' : ''" size="mini" icon="btn-approval"
								@click="approval(operate.row)"></el-button>
							<el-button :disabled="false"
							title="开证明" size="mini" :type="operate.row.ddhlczt == 40 ? '' : 'primary'" class="icon-intro iconfont"
              ...
              <el-button title="下载" type="primary" size="mini" icon="btn-download"
								@click="download(operate.row)"></el-button>
              <el-button title="打印" type="primary" size="mini" icon="btn-print"
...
export default {
	name: 'DanganLette',
	components: { dateAss, selectCode, letteCert },
	data() {
		return {
			labelWidth,
			color: {
	...
	methods: {
		...
		getData() {
			this.loading = true;
			$.get('/gyrcht/ldryda/ldryda/ddhsqCx', {
				params: this.form
			}).then((res) => {
				this.loading = false;
				this.results = res.returnData.ddhlb;
		...
		getEmailData() {
			this.loading = true;
			$.get('/gyrcht/ldryda/ldryda/ddhfsyjjbxxCX', {
				params: { kjddh_id: this.emailForm.kjzm_id }
			}).then((res) => {
				this.loading = false;
				const data = res.returnData.yjxx;
		...
		// 开证明
		openProof(row) {
			if (row.ddhlczt === '10' || row.ddhlczt === '20') {
		...
		// 发邮件
		sendEmail(row) {
			if (row.ddhlczt === '40' || row.ddhlczt === '50' || row.ddhlczt === '60') {
				this.emailForm.kjzm_id = row.kjddh_id;
				this.dialogEmail = true;
				this.isLczt = row.ddhlczt;
				if (row.ddhlczt === '50') {
					this.getEmailData();
				} else {
		...
		download(row) {
			if (row.ddhlczt === '40' || row.ddhlczt === '50' || row.ddhlczt === '60') {
				const url = `/gyrcht/ldryda/ldryda/ddhDc?${'kjddh_id'}=${row.kjddh_id}`;
				window.open(url);
			} else {
				this.$message.warning('您还未生成证明，请先生成证明！');
			}
		...
		uploadSuccess(res) {
			if (res.returnData.executeResult === '1') {
				...
				this.emailForm.lj = data.serverPath;
				this.emailForm.fwdmc = data.serverName;
				this.emailForm.khdmc = data.clientName;
		...
		putData() {
			this.loading = true;
			$.post('/gyrcht/ldryda/ldryda/ddhfsyjBc', this.emailForm)
				.then((res) => {
					if (res.returnData.executeResult === '1') {
						this.$message({
							type: 'success',
