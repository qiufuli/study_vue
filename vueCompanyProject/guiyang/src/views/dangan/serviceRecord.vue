...
<template>
	<div>
        ...
        <el-row>
          ...
          <el-col :span="6">
						<el-form-item label="证明类型" prop="kjzmlx">
              <!-- <select-code v-model="form.kjzmlx" code="gyrlzyw_ld_d_kjzmlx"></select-code> -->
							<el-select v-model="form.kjzmlx" class="width_100">
								<el-option value="" label="全部"></el-option>
								<el-option value="10" label="档案托管证明"></el-option>
								<el-option value="20" label="档案政审证明"></el-option>
								<el-option value="30" label="其他情况证明"></el-option>
								<el-option value="50" label="调档函"></el-option>
							</el-select>
						</el-form-item>
					</el-col>
        </el-row>
		...
		<div class="loading" v-loading="loading">
			<div class="result_wrap">
				<el-table :data="results" stripe class="table_wrap width_100">
					...
					<el-table-column label="身份证号码" prop="sfzh" min-width="150"></el-table-column>
					...
					<el-table-column label="发送邮件日期" prop="fsyjrq"></el-table-column>
					<el-table-column label="流程状态" prop="lcztmc"></el-table-column>
					<el-table-column label="证明类型" prop="kjzmlxmc"></el-table-column>
					<el-table-column label="操作" width="210">
						<template scope="operate">
							<!-- <el-button title="审核" :type="operate.row.lczt > 20 ? '' : (operate.row.lczt == 20 ? 'danger' : 'primary')" size="mini" icon="btn-approval"
								@click="approval(operate.row)"></el-button>
							<el-button :disabled="false"
							title="开证明" size="mini" :type="operate.row.lczt == 40 || operate.row.lczt == 50 ? '' : (operate.row.lczt == 20 ? 'danger' : 'primary')" class="icon-intro iconfont"
							@click="openProof(operate.row)"></el-button>
              <el-button title="发邮件" :type="operate.row.lczt == 50 ? '' : (operate.row.lczt == 20 ? 'danger' : 'primary')" size="mini" class="icon-mail iconfont"
		...
		@close="cancel('formInfo')">
			<el-form :model="emailForm" ref="formInfo" :rules="rulesInfo" :label-width="labelWidth">
				...
				<el-form-item label="附件上传" required>
					<el-upload
						ref="upload"
						action="/common/wjsc/wjsc/affixUpload?modeName=yjfj"
					...
					<ul class="file_list">
						<li v-for="(item, index) in ddhfj" :key="index">
		...
		<!--发送邮件 end-->
		<!-- 调档函打印 dialog 开始 -->
		<service-cert :visible.sync="dialogAcess" :kjzmId="kjzmId" :isSend="false" :enterList="true"></service-cert>
		<!-- 调档函打印 dialog 结束 -->
		<!-- 调档函打印 50 dialog 开始 -->
		<lette-cert :visible.sync="dialogLetAcess" :kjddhId="kjddhId"></lette-cert>
		<!-- 调档函打印 50 dialog 结束 -->
	</div>
</template>
<script>
...
export default {
	name: 'otherService',
	components: { dateAss, selectCode, serviceCert, letteCert },
	data() {
		return {
			labelWidth,
			color: {
	...
	methods: {
		...
		getEmailData() {
			this.loading = true;
			$.get('/gyrcht/kjzm/kjzm/zmfsyjbjCx', {
		...
		download(row) {
			if (row.lczt === '40' || row.lczt === '50' || row.lczt === '60') {
				if (row.kjzmlx === '50') {
					const url = `/gyrcht/ldryda/ldryda/ddhDc?${'kjddh_id'}=${row.kjzm_id}`;
					window.open(url);
				} else {
					const url = `/gyrcht/kjzm/kjzm/dcsczmCx?${'kjzm_id'}=${row.kjzm_id}`;
					window.open(url);
				}
				// const url = `/gyrcht/kjzm/kjzm/dcsczmCx?${'kjzm_id'}=${row.kjzm_id}`;
				// window.open(url);
		...
		print(row) {
			if (row.lczt === '40' || row.lczt === '50' || row.lczt === '60') {
				if (row.kjzmlx === '50') {
					this.dialogLetAcess = true;
					this.kjddhId = row.kjzm_id;
				} else {
		...
		putData() {
			this.loading = true;
			$.post('/gyrcht/kjzm/kjzm/zmfsyjBc', this.emailForm)
				.then((res) => {
					if (res.returnData.executeResult === '1') {
						this.$message({
							type: 'success',
