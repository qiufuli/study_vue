				...
				<el-row>
					...
					<el-col :span="12">
						<el-form-item label="录入日期">
							<date-ass :begin.sync="form.cjsjq" :end.sync="form.cjsjz"
							propBegin="cjsjq" propEnd="cjsjz"></date-ass>
						</el-form-item>
					</el-col>
					<el-col :span="6">
						<el-form-item label="录入人" prop="sprXm">
							<el-autocomplete v-model="form.sprXm"
		...
		<div class="loading" v-loading="loading">
			<div class="result_wrap">
				<el-table :data="results" stripe class="width_100 table_wrap">
					...
					<el-table-column label="录入日期" prop="cjsj"></el-table-column>
					<el-table-column label="录入人" prop="cjrmc"></el-table-column>
				</el-table>
			</div>
			<!-- 分页 -->
...
export default {
	...
	data() {
		return {
			...
			// 验证
			rules: {
				dwmc: [{ validator: validator('48, "bz", "单位名称", true') }],
				sprXm: [{ validator: validator('48, "bz", "录入人", true') }]
			},
			// 克隆表单（用于提交）
			formRight: {}
		};
	...
	methods: {
		...
		getData() {
			this.loading = true;
			$.get('/gyrcht/xtgl/xtgl/getZwlrjl', {
				params: this.formRight
			}).then((res) => {
				this.results = res.returnData.zwlrjl;
		...
		suggestionCompany(query, cb) {
			// 先进行数据校验
			this.$refs.form.validateField('sprXm', (errorMsg) => {
				// 数据为空不进行联想
				if (query === '') {
					return cb([]);
				}
